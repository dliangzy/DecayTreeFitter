		...
		class FitParams {
				...
				public:
						...
						FitParams(const FitParams& toCopy)
								...
								m_globalState(Eigen::Matrix < double, 1, -1, 1, 1, MAX_MATRIX_SIZE> (toCopy.m_globalState)),
				...
				{ }
						...
						Eigen::Matrix < double, 1, -1, 1, 1, MAX_MATRIX_SIZE> & getStateVector() {return m_globalState;}

						/** const getter for the fit parameters/statevector */
						const Eigen::Matrix < double, 1, -1, 1, 1, MAX_MATRIX_SIZE> & getStateVector() const
						{
								return m_globalState;
						}

						/** get a reference to an element of the state vector todo replace by setter? */
						double& getRefToElementOfStateVec(int col) { return m_globalState(0, col); }

						/** get a const reference to an element of the state vector todo replace by setter? */
						const double& getRefToElementOfStateVec(int col) const { return m_globalState(0, col); }
						...
						friend ostream& operator<<(ostream & os, const FitParams & fitparams);
				...
				private:
						...
						Eigen::Matrix < double, 1, -1, 1, 1, MAX_MATRIX_SIZE> m_globalState;
